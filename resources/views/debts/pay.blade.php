<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold text-white">Pay Debt</h2>
                <p class="text-sm text-slate-400 mt-1">{{ $debt->description }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-lg mx-auto">
        ...
        <div class="bg-zinc-900 border border-zinc-800/50 rounded-2xl p-6 mb-6">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-xs text-zinc-500">Total Debt</p>
                    <p class="text-lg font-semibold text-white">${{ number_format($debt->amount, 2) }}</p>
                </div>
                <div>
                    <p class="text-xs text-zinc-500">Already Paid</p>
                    <p class="text-lg font-semibold text-emerald-400">${{ number_format($debt->amount_paid, 2) }}</p>
                </div>
            </div>
            <div class="mt-4 pt-4 border-t border-zinc-800/50">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-400">Remaining</p>
                    <p class="text-2xl font-bold text-rose-400">${{ number_format($debt->remaining, 2) }}</p>
                </div>
        ...
        <div class="bg-zinc-900 border border-zinc-800/50 rounded-2xl p-6">
            <form method="POST" action="{{ route('debts.pay.store', $debt) }}">
                @csrf
                <div class="mb-6">
                    <label for="amount" class="block text-sm font-medium text-zinc-400 mb-2">Payment Amount ($)</label>
                    <input type="number" name="amount" id="amount" step="0.01" min="5" max="{{ $debt->remaining }}" value="{{ old('amount') }}" required
                        ...
                        placeholder="Min $5.00 — Max ${{ number_format($debt->remaining, 2) }}">
                    @error('amount')
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('debts.index') }}" class="px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white font-medium rounded-xl transition-all duration-200 text-center border border-zinc-700/50">
                        Cancel
                    </a>
                </div>
            </form>
