<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold text-white">Debt Details</h2>
        ...
        <div class="bg-zinc-900 border border-zinc-800/50 rounded-2xl p-6">
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">${{ number_format($debt->amount, 2) }}</p>
                </div>
                <div class="text-right">
                    <span class="inline-flex items-center px-2.5 py-1 border rounded-full text-xs font-medium {{ $debt->status === 'paid' ? 'bg-zinc-800 border-zinc-700 text-emerald-400' : 'bg-zinc-800 border-zinc-700 text-rose-400' }}">
                        {{ strtoupper($debt->status) }}
                    </span>
                    <p class="text-sm text-zinc-400 mt-2">Added on {{ $debt->created_at->format('M d, Y') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-zinc-800/50">
                ...
                <div class="text-right">
                    ...
                    <p class="text-lg font-semibold text-rose-400">${{ number_format($debt->remaining, 2) }}</p>
                </div>
            </div>
        </div>
                    ...
                    @foreach($debt->payments as $payment)
                        <div class="flex items-center justify-between p-4 bg-zinc-950 rounded-xl border border-zinc-800">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-white">Payment Made</p>
